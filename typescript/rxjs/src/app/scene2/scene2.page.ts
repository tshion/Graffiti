...
import { catchError, concatMap, concatMapTo, from, tap } from 'rxjs';
import { SecretResponse } from './secret.response';
...
export class Scene2Page implements OnInit {
  ...
  challenge2() {
    ...
    log("call token:post, start")
    this.http.post<TokenResponse>(this.urlToken(), null).pipe(
